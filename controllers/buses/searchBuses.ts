...
export default async function searchBuses(req: Request, res: Response): Promise<Response> {
    ...
    const data: { from?: string; to?: string; passengers?: string } | undefined = req.body
    ...
    try {
        ...
        const travelBusQs = `
            ...
            WHERE
                travel_departures_and_returns.depart_from = $1
                AND travel_departures_and_returns.depart_to = $2
                AND travel_departures_and_returns.date >= $3

                GROUP BY travel_list.id,
                travel_list.driver,
                travel_list.title,
                travel_list.description,
                travel_list.type,
        ...
        const travelBusRes: QueryResult = await pool.query(travelBusQs, [
            data.from,
            data.to,
            dateToday,
        ])
        ...
        if (!data.passengers || isNaN(Number(data.passengers)) || Number(data.passengers) <= 0) {
            return res.status(200).json(travelBusRows)
        }

        const filtered = travelBusRows?.filter((item) => {
            const elementsFiltered = item?.seats?.filter((seat) => !seat.booked)

            return elementsFiltered.length >= Number(data.passengers) ? true : false
        })
