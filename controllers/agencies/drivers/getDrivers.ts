...
export default async function getDrivers(req: Request, res: Response): Promise<Response> {
    const agencyId: undefined | string = req.params.id

    if (!agencyId || !uuidv4Regex.test(agencyId)) {
        return res.status(500).json({ message: "Please submit valid agency id" })
    }
