...
import getRequestToRefundStatus from "../../../configs/getRequestToRefundStatus"

export default async function requestToRefundStatus(
    req: Request,
    res: Response
): Promise<Response> {
    ...
    try {
        const paymentStatus = await getRequestToRefundStatus(data.referenceId, data.token)
